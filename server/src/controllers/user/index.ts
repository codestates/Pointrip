  ...
  getUserInfo: (req: Request, res: Response) => {
    ...
    return getRepo(User)
      ...
      .then(async (data: any) => {
        ...
        console.log(data[0]);
        const userPost = await getRepo(Post).createQueryBuilder().select('COUNT(*) AS A').where({ user: accessTokenData.id }).execute()
        const userStorage = await getRepo(Saved).createQueryBuilder().select('COUNT(*) AS A').where({ user: accessTokenData.id }).execute()
        
         
       
        res.json({ data: { userInfo: data[0],myPost: Number(userPost[0].A),
